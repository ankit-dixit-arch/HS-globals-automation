...
function readFormValues() {
  return {
    ...
    requireEmail: document.getElementById("requireEmail")?.checked || false,
...
function writeFormValues(profile) {
  ...
  const requireEmailEl = document.getElementById("requireEmail");
  if (requireEmailEl) {
    requireEmailEl.checked = !!profile.requireEmail;
  }
...
async function refreshStatus() {
  ...
  const phoneLabel = result.requirePhone ? " | Phone filter: ON" : " | Phone filter: OFF";
  const emailLabel = result.requireEmail ? " | Email filter: ON" : " | Email filter: OFF";
  const testLabel = result.testMode ? " | ⚠ TEST MODE ON (not clicking for real)" : "";

  document.getElementById("status").textContent =
    `Today: ${clicksToday} / ${maxClicks} clicks used.${activeLabel}${phoneLabel}${emailLabel}${testLabel} Make sure a BuyLeads tab is open.`;
}
