...
3. The extension now scans in three overlapping ways, so leads get caught
   ...
   - A MutationObserver watches the page itself and reacts the moment a
     ...
     a timer, so it isn't slowed down by the tab being hidden. If several
     leads arrive while another scan is running, it queues an immediate
     follow-up scan instead of dropping their trigger.
   - A 5-second timer re-scans everything currently on screen as a
